"""
SPY pbar scanner — Oct 1 2025 through Nov 30 2025, 5-min candles.
...
OUT = Path("data/pbar_spy_oct_nov2025.csv")
...
def main():
    ...
    for day in trading_days(START, END):
        ...
        for dt, c in kept:
            ...
            # Ghost vs PBAR classification.
            # Ghosts are tiny-size dark prints OR severely lagged late-reported
            # dark prints - ToS hides both.
            ...
            if ext_count > EXT_PRINT_MAX:
                verdict = "not_pbar"
            elif all_dark and ext_size < 1000 and (has_ghost_cond or max_lag_ms > 5000):
                verdict = "GHOST"
            else:
                verdict = "PBAR"
            ...
            print(f"  {flag} {row['date']} {row['time_et']} {direction:4} wick%={row['wick_pct']:.2f}"
                  ...
                  f"  ext={extreme}  ext_prints={ext_count}  ext_sz={ext_size:,}  lag={int(max_lag_ms)}ms  -> {verdict}")
    ...
    for r in pbars:
        print(f"  {r['date']} {r['time_et']} ET  {r['direction']:4}  wick%={r['wick_pct']:.2f}  "
              ...
              f"vol={r['volume']:,}  ext_prints={r['prints_at_extreme']}  ext_sz={r['prints_at_extreme_size']:,}")
    print(f"\nAll GHOSTs:")
    for r in ghosts:
        print(f"  {r['date']} {r['time_et']} ET  {r['direction']:4}  wick%={r['wick_pct']:.2f}  "
              f"O={r['open']} H={r['high']} L={r['low']} C={r['close']}  ext={r['extreme']}  "
              f"ext_prints={r['prints_at_extreme']}  ext_sz={r['prints_at_extreme_size']:,}  lag={r['max_extreme_lag_ms']}ms")
    print(f"\nWrote {OUT}")
