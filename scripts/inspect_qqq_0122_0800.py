"""Inspect QQQ 2026-01-22 08:00 ET 5-min candle. The scanner claims low=616.58."""
import os
...
# Get all 1-min bars for 2026-01-22
r = S.get(f"{BASE}/v2/aggs/ticker/QQQ/range/1/minute/2026-01-22/2026-01-22",
          params={"adjusted": "true", "sort": "asc", "limit": 50000}, timeout=60)
bars = r.json().get("results", [])
print(f"Got {len(bars)} 1-min bars for 2026-01-22\n")

# 07:55 - 08:30 ET
print("1-min bars 07:55 to 08:30 ET:")
target = datetime(2026,1,22,8,0).replace(tzinfo=ET)
t_start_ms = int((target - timedelta(minutes=5)).timestamp() * 1000)
t_end_ms = int((target + timedelta(minutes=30)).timestamp() * 1000)
for b in bars:
...
if in_bucket:
    o = in_bucket[0]["o"]; h = max(b["h"] for b in in_bucket); l = min(b["l"] for b in in_bucket); c = in_bucket[-1]["c"]
    v = sum(b.get("v",0) for b in in_bucket)
    body_top = max(o,c); body_bot = min(o,c)
    print(f"\n5-min bucket 08:00-08:05 ET aggregated from 1-min bars:")
    print(f"  O={o:.3f} H={h:.3f} L={l:.3f} C={c:.3f} V={v:,}")
    print(f"  body {body_bot:.3f}-{body_top:.3f}  upper wick {h-body_top:.3f}  lower wick {body_bot-l:.3f}")
    print(f"  upper wick% = {(h-body_top)/c*100:.3f}%   lower wick% = {(body_bot-l)/c*100:.3f}%")

# Trade tape 08:00-08:05
...
if trades:
    ...
    total_sz = sum(sizes); total_not = sum((t.get('size') or 0) * (t.get('price') or 0) for t in trades)
    print(f"  total vol: {total_sz:,}   notional: ${total_not/1e6:.2f}M")
    # Show lowest 10 prints
