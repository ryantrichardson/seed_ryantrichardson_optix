"""
PBAR scanner v2 — reusable, configurable by command-line args.

Usage:
  TICKER=SPY START=2025-10-01 END=2025-11-30 python pbar_scanner_v2.py

All known bug fixes:
  - DST: real-ET via zoneinfo('America/New_York'), not fixed UTC-5
  - Avg-price trades excluded (conds 2 and 52)
  - GHOST detection refined: any cond in {7, 22, 32, 53} on a dark print OR
    sip->participant lag > 5000ms now flags GHOST regardless of size
  - Same-session repeat-level flag added

Follows v3 baseline: Bucket trades by sip_timestamp (report time), not participant_timestamp
...
Adds GHOST verdict alongside PBAR using two discriminators learned from
January 2026 ghost-vs-pbar comparison:
  - Total share size at extreme < 1000 shares (ghosts: 100-140 sh, pbars: 25K-2.3M)
  - All extreme prints are dark (trf_id present), AND
  - Any extreme print has condition in {7, 32} (Form-T late variants) OR has
    sip_timestamp - participant_timestamp lag > 5000 ms.
Exchange-listed premarket trades (trf_id None) stay as PBAR even at low size,
because ToS shows them.

Output: data/pbar_spy_oct_nov2025.csv
"""
...
START = date.fromisoformat(os.environ.get("START", "2025-10-01"))
END   = date.fromisoformat(os.environ.get("END",   "2025-11-30"))
WICK_MIN = 0.5
WICK_MAX = 5.0
EXT_PRINT_MAX = 20
TOL = 0.05
...
OUT = Path(f"data/pbar_{TICKER.lower()}_{START.isoformat()}_{END.isoformat()}.csv")
...
def main():
    ...
    for day in trading_days(START, END):
        ...
        # Track all candles within this day for repeat-level lookups.
        # We update this BEFORE wick filtering so non-wick candles also contribute.
        same_day_priors = []
        for dt, c in kept:
            cur_session = session_label(dt)
            cl = classify_wick(c)
            if not cl:
                same_day_priors.append({"dt": dt, "session": cur_session, "h": c["h"], "l": c["l"]})
                continue
            ...
            # Ghost vs PBAR classification (v2 rules).
            # GHOST signature = late-reported dark print(s):
            #   - all prints at extreme are dark (trf_id present)
            #   - any print has cond in {7, 22, 32, 53} (Form-T late variants)
            #     OR has sip->participant lag > 5000ms
            # Visible PBARs are either exchange-listed (trf=None) or cond 12/14
            # (normal Form-T) with sub-second lag.
            all_dark = ext_prints and all((t.get("trf_id") is not None) for t in ext_prints)
            has_ghost_cond = False
            max_lag_ms = 0
            for t in ext_prints:
            ...
            # Same-session repeat-level flag:
            # Was this extreme price already inside the H-L range of an earlier
            # candle the same session? If so, flag it for review.
            repeat_level = False
            for prior in same_day_priors:
                if prior["session"] != cur_session:
                    continue
                if prior["l"] - TOL <= extreme <= prior["h"] + TOL:
                    repeat_level = True
                    break
            ...
            rows.append(row)
            # add this candle's range to priors AFTER processing (so we don't
            # mark the current candle as self-repeating)
            same_day_priors.append({"dt": dt, "session": cur_session, "h": c["h"], "l": c["l"]})
            flag = {"PBAR": "★", "GHOST": "☆"}.get(verdict, " ")
            rl = " [repeat]" if repeat_level else ""
            print(f"  {flag} {row['date']} {row['time_et']} {direction:4} wick%={row['wick_pct']:.2f}"
                  f"  H={row['high']} L={row['low']}  ext={extreme}"
                  f"  ext_prints={ext_count} ext_sz={ext_size:,} lag={int(max_lag_ms)}ms"
                  f"  -> {verdict}{rl}")
    ...
    print(f"\n{'='*80}\nSUMMARY: {len(rows)} candidates, {len(pbars)} PBARs, {len(ghosts)} GHOSTs\n{'='*80}")
    print(f"PBARs by direction:  up={sum(1 for r in pbars if r['direction']=='up')}  down={sum(1 for r in pbars if r['direction']=='down')}")
    print(f"PBARs by session:    pre={sum(1 for r in pbars if r['session']=='pre')}  rth={sum(1 for r in pbars if r['session']=='rth')}  post={sum(1 for r in pbars if r['session']=='post')}")
    print(f"\nAll PBARs (slim):")
    print(f"  {'date':<10} {'time':<5} {'dir':<4} {'wick%':<6} {'high':<10} {'low':<10} {'extreme':<10} {'repeat':<6}")
    for r in pbars:
        rl = "yes" if r.get("repeat_level") else ""
        print(f"  {r['date']:<10} {r['time_et']:<5} {r['direction']:<4} {r['wick_pct']:<6.2f} "
              f"{r['high']:<10} {r['low']:<10} {r['extreme']:<10} {rl:<6}")
    print(f"\nAll GHOSTs (slim):")
    print(f"  {'date':<10} {'time':<5} {'dir':<4} {'wick%':<6} {'high':<10} {'low':<10} {'extreme':<10} {'repeat':<6}")
    for r in ghosts:
        rl = "yes" if r.get("repeat_level") else ""
