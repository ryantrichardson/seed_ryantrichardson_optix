"""
SPY pbar scanner v2 — January 2026, 5-min candles.

KEY FIX: Build 5-min OHLC directly from /v3/trades (including condition 12
Form-T and 22 Form-T-late dark prints, which ToS displays but Massive's
/v2/aggs endpoint filters out). This matches the chart Ryan sees on ToS.
...
Output: data/pbar_spy_jan2026_v2.csv
"""
import os, csv, time
...
OUT = Path("data/pbar_spy_jan2026_v2.csv")
...
def build_5min_candles_from_trades(trades, day_start_ms):
    ...
    for t in trades:
        if not is_tos_visible(t):
            continue
        # Use participant timestamp (when trade happened) - that's what shows on chart
        ts = t.get("participant_timestamp") or t.get("sip_timestamp")
        if not ts:
            continue
    ...
    for day in trading_days(START, END):
        ...
        for dt, c in kept:
            ...
            window_trades = [t for t in trades
                             if bkt_start_ns <= int(t.get("participant_timestamp") or t.get("sip_timestamp") or 0) < bkt_end_ns]
            if direction == "down":
