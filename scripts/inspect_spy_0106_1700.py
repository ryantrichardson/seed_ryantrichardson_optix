"""Inspect SPY 2026-01-06 17:00 ET 5-min candle."""
import os
...
r = S.get(f"{BASE}/v2/aggs/ticker/SPY/range/1/minute/2026-01-06/2026-01-06",
          params={"adjusted": "true", "sort": "asc", "limit": 50000}, timeout=60)
bars = r.json().get("results", [])
print(f"Got {len(bars)} 1-min bars for SPY 2026-01-06\n")

target = datetime(2026,1,6,17,0).replace(tzinfo=ET)
t_start_ms = int((target - timedelta(minutes=5)).timestamp() * 1000)
t_end_ms = int((target + timedelta(minutes=10)).timestamp() * 1000)
print("1-min bars 16:55 - 17:10 ET:")
for b in bars:
...
print(f"\nTrade tape 17:00-17:05 ET: {len(trades)} trades")
if trades:
