"""Adapted PBAR scanner for the spy_extend_v2 artifacts.
Reads /tmp/artifacts/shard*/slim_trades_spy_ext_shard*.csv.gz (8 shards).
Writes data/pbar_spy_may_nov2024.csv to the repo root.
"""
...
# Inputs: all 8 shard csv.gz files
SHARDS = sorted(glob.glob("/tmp/artifacts/spy_ext_v2_shard*/slim_trades_spy_ext_shard*.csv.gz"))
print(f"Found {len(SHARDS)} shards:")
for s in SHARDS: print(f"  {s}")

OUT = Path("data/pbar_spy_may_nov2024.csv")
OUT.parent.mkdir(exist_ok=True, parents=True)
PKL_DIR = Path("/tmp/per_day_spy_ext")
...
def phase1():
    print(f"=== Phase 1: partition shards by day ===", flush=True)
    for shard in SHARDS:
        t0 = time.time()
