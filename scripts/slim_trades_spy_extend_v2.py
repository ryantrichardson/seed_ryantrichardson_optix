"""Extension v2: stream SPY trades from May 2024 - Nov 2024.
Hardened with:
  - Per-day hard timeout via SIGALRM (kills stuck days, doesn't hang the whole runner)
  - 8 shards (vs original 4) - smaller per-shard load
  - Verbose progress every day
  - Same trade-level columns as Dec 2024-Apr 2025 data (apples-to-apples PBAR/GHOST classification)
"""
import os
import gzip
import io
...
# Per-day hard cap: 8 minutes. If a single day takes longer than this, skip it.
...
s3 = boto3.client(
    ...
    config=Config(
        ...
        read_timeout=180,    # tighter than v1 (was 600) so we fail-fast on hung reads
        connect_timeout=20,
    ),
)
...
def process_day(d, writer):
    """Stream and filter one day. Returns (kept, scanned) or (None,None)."""
    ...
    try:
        ...
        try:
            ...
            return kept, scanned  # keep partial
        except (ReadTimeoutError, ConnectTimeoutError) as e:
            print(f"  {d} stream-error mid-read: {e}; kept {kept:,} so far")
            try: text_stream.close()
            except: pass
            return kept, scanned
        except Exception as e:
            print(f"  {d} unexpected mid-read: {e}; kept {kept:,} so far")
            try: text_stream.close()
            except: pass
            return kept, scanned
    finally:
...
with gzip.open(OUT_PATH, "wt", encoding="utf-8", newline="") as out_f:
    ...
    for d in my_days:
        ...
        try:
            ...
            print(f"  {d} HARD-TIMEOUT after {elapsed:.0f}s — skipped")
            continue
        except Exception as e:
