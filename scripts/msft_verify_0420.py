...
def get_trades(ticker, day_str):
    ...
    start = datetime(d.year, d.month, d.day, 13, 0, tzinfo=ET)
    end   = datetime(d.year, d.month, d.day, 14, 30, tzinfo=ET)
...
print(f"Total trades 13:00-14:30: {len(trades)}")

# Build 1-min bars (excluding noise conditions)
...
# Print bars 13:35-13:55
print("\n=== 1-min bars 13:35-13:55 ET ===")
print(f"{'Minute':6} {'O':>8} {'H':>8} {'L':>8} {'C':>8} {'Trades':>7} {'Vol':>8}")
for minute in sorted(by_minute):
    if minute.hour == 13 and (minute.minute < 35 or minute.minute > 55): continue
    if minute.hour != 13: continue
...
# Find the lowest INCLUDED trade in window 13:40-13:50
incl = [t for t in trades if not (set(t["conditions"]) & {2,12,16,33,52,53})]
window = [t for t in incl if t["t"].hour == 13 and 40 <= t["t"].minute <= 50]
print(f"\n=== Lowest 30 INCLUDED trades in 13:40-13:50 window ({len(window)} total) ===")
for t in sorted(window, key=lambda x: x["price"])[:30]:
    tag = ""
...
# Also include ALL trades (incl. noise-excluded) at the extreme
print(f"\n=== ALL trades (incl noise-excluded) below $418 in 13:40-13:50 ===")
allwin = [t for t in trades if t["t"].hour == 13 and 40 <= t["t"].minute <= 50 and t["price"] < 418]
for t in sorted(allwin, key=lambda x: x["price"])[:30]:
    excluded = bool(set(t["conditions"]) & {2,12,16,33,52,53})
    tag = " [NOISE-EXCLUDED]" if excluded else ""
    if t["exchange"] == 4 and 37 in t["conditions"] and 41 in t["conditions"] and t["size"] < 100:
        tag += " [TRF-EXEMPT odd-lot]"
    elif t["exchange"] == 4:
        tag += " [TRF]"
    if 14 in t["conditions"]:
        tag += " [ISO SWEEP]"
    print(f"  {t['t'].strftime('%H:%M:%S.%f')[:-3]}  ${t['price']:>8.4f}  sz={t['size']:>5}  ex={t['exchange']:>3}  cond={t['conditions']}  trf={t.get('trf_id')}{tag}")

# Run ghost wick detector on these bars
print("\n=== Running ghost wick detector on 13:00-14:30 ===")
bars = []
for minute in sorted(by_minute):
    prices = [tr["price"] for tr in by_minute[minute]]
...
for i, b in enumerate(bars):
    ...
    for direction, wick in [("up", upper), ("down", lower)]:
        ...
        if et and et["exchange"] == 4 and 37 in et["conditions"] and 41 in et["conditions"] and et["size"] < 100:
            ...
            tag = " [TRF]"
        elif et and 14 in et["conditions"]:
            tag = " [ISO SWEEP]"
        print(f"  {b['t'].strftime('%H:%M')}  {direction.upper():4} ${extreme:.4f} ({wick_pct:.3f}%, ratio {ratio:.1f}, ext_size {et['size'] if et else '?'}, ex={et['exchange'] if et else '?'}){tag}")
